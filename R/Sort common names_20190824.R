...
sci_to_common(fish[which(IUCNStatWeb[,2]=="")])
IUCNStatWeb$CommonName[which(IUCNStatWeb[,2]=="")] <- as.character(sci_to_common(fish[which(IUCNStatWeb[,2]=="")]))
...
dim(IUCNStatWeb)
dim(specinfo)
which(duplicated(specinfo[,1])==TRUE) # At least there's no duplicates!

#Extract stock info and merge with other dataframe
resil <- stocks(fish, fields="Resilience")
specresil <- merge(specinfo, resil)
...
which(duplicated(specresil[,1])==TRUE) # Lot's of duplicates this time round but visual inspection says we can remove the second ones as they have an NA value

specresil <- specresil[!duplicated(specresil[,1]),]

IUCNFishbaseWeb <- merge(IUCNStatWeb,specresil,
                         by.x="ScientificName",by.y="Species",all.x = TRUE)
IUCNFishbaseWeb <- IUCNFishbaseWeb[order(IUCNFishbaseWeb[,1]),]
...
# Make DF smaller to make joining faster
IUCNFishbaseWeb1 <- IUCNFishbaseWeb %>%
  select(ScientificName,CommonName2,
...
IUCNFishbaseWeb1 <- left_join(IUCNFishbaseWeb,Df_MarkRecapt)



# Save the file for loading into shiny
write.csv(IUCNFishbaseWeb,"Data/IUCNFishbaseWeb.csv",row.names=FALSE)
